...
export class CvPaymentRequestListServiceComponent implements OnInit {
  ...
  ngOnInit() {
    this.titleService.setTitle(" Acct-Serv | Payment Requests List");
    this.getCvPaytReqList();
  }

  getCvPaytReqList(){
    var subRes = forkJoin(this.accountingService.getAcseCv(this.passData.tranId),this.accountingService.getAcseCvPaytReqList(this.passData.tranId))
                       .pipe(map(([cv, pr]) => { return { cv, pr }; }));

    subRes.subscribe(data => {
      console.log(data);
      var recCv = data['cv']['acseCvList'][0];
      var recPr = data['pr']['acseCvPaytReqList'];
  ...
  onSavePaytReqList(){
    ...
    this.accountingService.saveAcseCvPaytReqList(JSON.stringify(this.params))
    .subscribe(data => {
      console.log(data);
      this.getCvPaytReqList();
      this.suc.open();
  ...
  onClickViewDetails(){
    setTimeout(() => {
      this.router.navigate(['/acc-s-generate-request', { tranId : this.passData.tranId ,reqId : this.paytData.reqId , from: 'cv-paytreqlist' }], { skipLocationChange: true });
    },100);
  }
