...
export class AccSEditAccountingEntriesComponent implements OnInit, OnDestroy {
  ...
  setSelectedData(data){
    ...
    switch(this.tranClass){
      case 'OR':
        this.tranNo = data.data.formattedOrNo;
        break;
      case 'CV':
    ...
    /*this.accountingService.getAcitAcctEntries(data.data.tranId).subscribe(
      (data: any)=>{
        console.log(data);
        this.passData.tableData = data.acctEntries;
  ...
  retrieveData(tranId){
    ...
    var sub$ = forkJoin(this.accountingService.getAcseAcctEntries(tranId),
                        this.accountingService.getAcseEditedAcctEntries(tranId))
    ...
    this.forkSub = sub$.subscribe(
      (forkData:any)=>{
        ...
        this.passData.tableData = acctEntries.acctEntries.map(a=>{a.showMG = 1; return a;});
        this.table.refreshTable();
        this.table.overlayLoader = false;
  ...
  clickLov(data){
    ...
    if(data.key == 'glShortCd'){
      this.passLovAe.selector = 'acseChartAcct';
  ...
  save(){
    this.savedData = this.passData.tableData.filter(a=>a.edited && !a.deleted).map(b=>{b.createUser = this.ns.getCurrentUser();
      ...
      this.accountingService.editAcctEntServ(params).subscribe(a=>{
        if(a['returnCode']==0){
          this.dialogIcon = 'error';
          this.successDiag.open();
        }else{
  ...
  restore(){
    ...
    this.accountingService.restoreAcctEntServ(params).subscribe(a=>{
        if(a['returnCode']==0){
          this.dialogIcon = 'error-message';
