...
export class AcctItEditedAcctEntriesComponent implements OnInit, OnDestroy {
  ...
  retrieveAcctEntInq(){
    ...
    this.accountingService.getAcitAcctEntInq(this.filters.tranClass, this.filters.tranDateFrom, this.filters.tranDateTo).subscribe(
      (data:any)=>{
  ...
  onRowClick(data){
    ...
    if(data !== null){
      ...
      switch(data.tranClass){
        case 'AR':
          this.information.tranClassDesc = 'Acknowledgement Receipt';
          break;
        case 'JV':
  ...
  showModal(content) {
    ...
    var sub$ = forkJoin(this.accountingService.getAcitAcctEntries(this.information.tranId),
                        this.accountingService.getAcitAcctEntBackup(this.information.tranId,this.information.histNo)).pipe(map(([newAcctEnt, oldAcctEnt]) => { return { newAcctEnt, oldAcctEnt }; }));
    this.sub = sub$.subscribe(
      (forkData:any)=>{
        ...
        this.passDataLatestAcctEntries.tableData = newAcctEnt.list;
