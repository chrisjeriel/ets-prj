...
export class UnappliedCollectionComponent implements OnInit {
  ...
  export(){
    ...
    alasql('CREATE TABLE sample(row1 VARCHAR2, row2 VARCHAR2, row3 VARCHAR2, row4 VARCHAR2, row5 VARCHAR2, row6 VARCHAR2, row7 VARCHAR2, row8 VARCHAR2)');
    ...
    alasql('INSERT INTO sample VALUES (?,?,?,?,?,?,?,?)', ['Type','Item', 'Reference No.', 'Description', 'Currency', 'Curr Rate', 'Amount', 'Local Amount']);
    for(var i of this.passData.tableData){
      //totalCredit += i.creditAmt;
      //totalDebit += i.debitAmt;
      currAmt       += i.currAmt;
      localAmt      += i.localAmt;
      alasql('INSERT INTO sample VALUES(?,?,?,?,?,rate(?), currency(?), currency(?))', [i.transdtlName, i.itemName, i.refNo, i.remarks, i.currCd, i.currRate, i.currAmt, i.localAmt]);
    }
    //alasql('INSERT INTO sample VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', ['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '']);
    alasql('INSERT INTO sample VALUES (?,?,?,?,?,?,currency(?), currency(?))', ["","","", "", "", "TOTAL", currAmt, localAmt]);
    alasql('SELECT row1, row2, row3, row4, row5, row6, row7, row8 INTO XLSXML("'+filename+'",?) FROM sample', [mystyle]);
