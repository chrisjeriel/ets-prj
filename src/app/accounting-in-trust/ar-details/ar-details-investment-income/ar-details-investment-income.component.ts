...
export class ArDetailsInvestmentIncomeComponent implements OnInit {
 ...
 @Input() invData: any[] = [];
  ...
  passData: any = {
    tableData:[],
    tHeader:['Investment Code','Certificate No.','Investment Type','Security', 'Maturity Period', 'Duration Unit','Interest Rate','Date Purchased','Maturity Date','Curr','Curr Rate','Investment Income','Bank Charge','Withholding Tax', 'Net Income'],
    dataTypes:['text','text','text','text','number','text','percent','date','date','text','percent','currency','currency','currency','currency'],
    total:[null,null,null,null,null,null,null,null,null,null,'Total','incomeAmt','bankCharge','whtaxAmt', 'netIncome'],
    addFlag:true,
    deleteFlag:true,
    infoFlag:true,
    paginateFlag:true,
    ...
    keys: ['invtCode', 'certNo', 'invtTypeDesc', 'securityDesc', 'maturityPeriod', 'durationUnit', 'interestRate', 'purchasedDate', 'maturityDate', 'currCd', 'currRate', 
           'incomeAmt', 'bankCharge', 'whtaxAmt', 'netIncome'],
    uneditable: [false, true, true, true, true, true,true, true, true, true, true, true, true, true, true ],
    checkFlag: true,
    pageID: 6,
    widths:[220, 150, 1, 150, 1, 1, 85, 1, 1, 1, 85, 120, 120, 120, 120, 120]
  }
  ...
  retrieveIncomePullout(){
    this.passData.tableData = [];
    this.accountingService.getAcitArInvPullout(this.record.tranId, 1, 'I').subscribe(  //F is pullout type for this screen. Bill Id is always 1 for Investment Pullout
      (data: any)=>{
  ...
  save(cancelFlag?){
    ...
    for (var i = 0 ; this.passData.tableData.length > i; i++) {
      ...
      if(this.passData.tableData[i].edited && !this.passData.tableData[i].deleted){
         ...
         this.deletedData[this.deletedData.length-1].billId = 1; //1 for Investment Pull out Transaction Type
    ...
    let params: any = {
      ...
      billId: 1, //1 Investment Pull out Transaction Type
    ...
    this.accountingService.saveAcitArInvPullout(params).subscribe(
      (data:any)=>{
       if(data.returnCode === -1){
          ...
          this.dialogIcon = 'error-message';
          this.dialogMessage = 'The total Net Income of the Investment Income Pull-out must not exceed the AR Amount.';
          this.successDiag.open();
          if(this.cancelFlag){
            this.cancelFlag = false;
          }
  ...
  export(){
    ...
    alasql('CREATE TABLE sample(row1 VARCHAR2, row2 VARCHAR2, row3 VARCHAR2, row4 VARCHAR2, row5 VARCHAR2, row6 VARCHAR2, row7 VARCHAR2, row8 VARCHAR2, row9 VARCHAR2, row10 VARCHAR2,'+
                                'row11 VARCHAR2, row12 VARCHAR2, row13 VARCHAR2, row14 VARCHAR2, row15 VARCHAR2)');
    ...
    alasql('INSERT INTO sample VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', ['Income Pull-out', '', '', '', '', '', '', '', '', '', '', '', '', '', '']);
    alasql('INSERT INTO sample VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', ['Investment Code','Certificate No.','Investment Type','Security', 'Maturity Period', 'Duration Unit','Interest Rate','Date Purchased','Maturity Date','Curr','Curr Rate','Investment Income','Bank Charge','Withholding Tax','Maturity Value']);
    for(var i of this.passData.tableData){
      ...
      alasql('INSERT INTO sample VALUES(?,?,?,?,?,?,rate(?), datetime(?), datetime(?), ?, rate(?), currency(?), currency(?), currency(?), currency(?), currency(?))', [i.invtCode, i.certNo, i.invtTypeDesc, i.securityDesc, i.maturityPeriod, i.durationUnit, i.interestRate, i.purchasedDate, i.maturityDate, i.currCd, i.currRate, 
           i.incomeAmt, i.bankCharge, i.whtaxAmt, i.maturityValue]);
    }
    //alasql('INSERT INTO sample VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', ['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '']);
    alasql('INSERT INTO sample VALUES (?,?,?,?,?,?,?, ?, ?, ?, ?, currency(?), currency(?), currency(?), currency(?))', ["", "", "", "", "","", "", "", "", "","TOTAL" ,incomeAmt ,bankCharge ,whtaxAmt ,maturityValue]);
    alasql('SELECT row1, row2, row3, row4, row5, row6, row7, row8, row9, row10, row11, row12, row13, row14, row15 INTO XLSXML("'+filename+'",?) FROM sample', [mystyle]);
