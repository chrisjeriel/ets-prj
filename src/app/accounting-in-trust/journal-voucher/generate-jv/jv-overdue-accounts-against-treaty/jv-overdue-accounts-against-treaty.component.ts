...
////////////// END OLD
  ...
  ngOnInit() {
    ...
    // this.passDataOffsetting = this.accountingService.getInwardPolicyKeys('JV');
    this.passDataOffsetting.nData = {showMG:1,tranId : '',qsoaId : '',itemNo : '',policyId : '',policyNo : '',soaNo : '',coRefNo : '',effDate : '',dueDate : '',instNo : '', insuredDesc: '',currCd : '',currRate : '',premAmt : '',riComm : '',riCommVat : '',charges : '',netDue : '',prevPaytAmt : '',balPaytAmt : '',overdueInt : '',remarks : '',createUser : this.ns.getCurrentUser(),createDate : '',updateUser : this.ns.getCurrentUser(),updateDate : ''};
    if(this.jvDetail.statusType == 'N'){
      ...
      this.passData.disableAdd = true;
      this.passDataOffsetting.tHeaderWithColspan = this.passDataOffsetting.tHeaderWithColspan.slice(1);
      this.passDataOffsetting.uneditable = [true,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true];
  ...
  setSoa(data){
    ...
    for (var i = 0; i < data.data.length; i++) {
      ...
      this.quarterTable.indvSelect.acctOffset[this.quarterTable.indvSelect.acctOffset.length - 1].insuredDesc = data.data[i].insuredDesc;
    ...
    for( var i = 0 ; i < this.passData.tableData.length; i++){
      ...
      if(this.passData.tableData[i].deleted){
        if(this.passData.tableData[i].acctOffset.length == 0){
          ...
          for (var a = 0; a < this.passData.tableData[i].acctOffset.length; a++) {
           ...
           this.jvDetails.delAcctTrty[this.jvDetails.delAcctTrty.length - 1].quarterNo   =  this.jvDetails.delAcctTrty[this.jvDetails.delAcctTrty.length - 1].qsoaId;
          }
        }
      }
      ...
      for (var j = 0; j < this.passData.tableData[i].acctOffset.length; j++) {
        if(this.passData.tableData[i].acctOffset[j].edited && !this.passData.tableData[i].acctOffset[j].deleted && !this.passData.tableData[i].deleted){
          ...
          this.jvDetails.saveInwPolOffset[this.jvDetails.saveInwPolOffset.length - 1].quarterNo  = this.passData.tableData[i].qsoaId;
          ...
          this.jvDetails.saveInwPolOffset[this.jvDetails.saveInwPolOffset.length - 1].netDue     = this.passData.tableData[i].acctOffset[j].premAmt - this.passData.tableData[i].acctOffset[j].riComm - this.passData.tableData[i].acctOffset[j].riCommVat + this.passData.tableData[i].acctOffset[j].charges;
        }
        ...
        if(this.passData.tableData[i].acctOffset[j].deleted){
          ...
          this.jvDetails.delInwPolOffset[this.jvDetails.delInwPolOffset.length - 1].updateDate = this.ns.toDateTimeString(this.passData.tableData[i].acctOffset[j].updateDate)

          this.jvDetails.delInwPolOffset[this.jvDetails.delInwPolOffset.length - 1].quarterNo = this.passData.tableData[i].qsoaId;
        }
      }
      if(!this.passData.tableData[i].deleted){
